...
class Payment_Notice(models.Model):
    ...
    rent = models.DecimalField(max_digits=20, decimal_places=3,null=True)
    ...
    @classmethod
    def create_notice(cls, contract, ddl, start_date, end_date, rent, tta, nid, is_active, cname, loc):
        payment_notice = cls(contract=contract, deadline=ddl, payment_start_date=start_date, payment_end_date=end_date, 
        notice_id=nid, rent=rent, total_amount=tta, is_active=True, cname=cname, loc_code=loc)
